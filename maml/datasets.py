...
from torchmeta.datasets import Omniglot, MiniImagenet, TripleMNIST, DoubleMNIST
from torchmeta.toy import Sinusoid
from torchmeta.transforms import ClassSplitter, Categorical, Rotation
from torchvision.transforms import ToTensor, Resize, Compose

from maml.model import ModelConvOmniglot, ModelConvMiniImagenet, ModelConvDoubleMNIST, ModelMLPSinusoid
...
def get_benchmark_by_name(name,
    ...
    if name == 'sinusoid':
    ...
    elif name == 'triplemnist':
        transform = Compose([Resize(84), ToTensor()])

        meta_train_dataset = TripleMNIST(folder,
                                          transform=transform,
                                          target_transform=Categorical(num_ways),
                                          num_classes_per_task=num_ways,
                                          meta_train=True,
                                          dataset_transform=dataset_transform,
                                          download=True)
        meta_val_dataset = TripleMNIST(folder,
                                        transform=transform,
                                        target_transform=Categorical(num_ways),
                                        num_classes_per_task=num_ways,
                                        meta_val=True,
                                        dataset_transform=dataset_transform)
        meta_test_dataset = TripleMNIST(folder,
                                         transform=transform,
                                         target_transform=Categorical(num_ways),
                                         num_classes_per_task=num_ways,
                                         meta_test=True,
    ...
    elif name == 'doublemnist':
        transform = Compose([Resize(64), ToTensor()])

        meta_train_dataset = DoubleMNIST(folder,
                                          transform=transform,
                                          target_transform=Categorical(num_ways),
                                          num_classes_per_task=num_ways,
                                          meta_train=True,
                                          dataset_transform=dataset_transform,
                                          download=True)
        meta_val_dataset = DoubleMNIST(folder,
                                        transform=transform,
                                        target_transform=Categorical(num_ways),
                                        num_classes_per_task=num_ways,
                                        meta_val=True,
                                        dataset_transform=dataset_transform)
        meta_test_dataset = DoubleMNIST(folder,
                                         transform=transform,
                                         target_transform=Categorical(num_ways),
                                         num_classes_per_task=num_ways,
                                         meta_test=True,
        ...
        model = ModelConvDoubleMNIST(num_ways, hidden_size=hidden_size)
        loss_function = F.cross_entropy
